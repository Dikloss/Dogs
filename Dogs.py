...
def get_dog_image():
    try:
        ...
        mb.showerror("Ощибка", f"Возникла ошибка при запросе к API {e}")
        return None
...
def show_image():
    ...
    if image_url:
        try:
            ...
            img_size = (int(width_spinbox.get()), int(width_spinbox.get()))
            ...
            lb.image = img # позволяет сохранить картинкуБ чтобы мусорщик не забрал
        except Exception as e:
            mb.showerror("Ошибка", f"Возникла ошибка при загрузке {e}")
    progress.stop()
